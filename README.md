<!DOCTYPE html>
<html>
  <head>
    <title>Desmoto's Arts & Craft</title>
  </head>

  <body>
    <link href="style.css" rel="stylesheet" id="style">
    <div class="pattern">
    <h1>Desmoto's Arts & Craft</h1>
    
    </div>
    <div>
      <h2 id="brushes">Brushes</h2>
      <img src="//content.codecademy.com/courses/freelance-1/unit-2/hacksaw.jpeg" alt="Hacksaw Brushes">
      <h3 id="hacksaw">Hacksaw Brushes</h3>
      <p>Made of the highest quality oak, Hacksaw brushes are known for their weight and ability 
        to hold paint in large amounts. Available in different sizes. 
        <strong>Starting at $5.00 / tube.</strong></p>
    </div>
    <div>
      <h2 id="frames">Frames</h2>
      <img src="//content.codecademy.com/courses/freelance-1/unit-2/frames.jpeg" alt="Frames">
    <h3>Art Frames (assorted)</h3>
    <p>Assorted frames made of different material, including MDF, birchwood, and PDE. 
        Select frames can be sanded and painted according to your needs. 
        <strong>Starting at $5.00 / tube.</strong></p>
    </div>
    <div>
        <h2 id="paint">Paint</h2>
        <img src="//content.codecademy.com/courses/freelance-1/unit-2/finnish.jpeg" alt="Finnish Paint">
        <h3>Clean Finnish Paint</h3>
        <p>Imported paint from Finland. Over 256 colors available in-store, varying in quantity (1 oz. to 8 oz.). 
        Clean Finnish paint microbinds to canvas, increasing the finish and longevity of any artwork. 
        <strong>Starting at $5.00 / tube.</strong></p>
        </div>
  </body>
</html>
